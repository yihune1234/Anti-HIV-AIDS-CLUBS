...
class ResourceController {
    ...
    async markCompleted(req, res) {
        try {
            const resource = await resourceService.markCompleted(req.params.id, req.user._id, req.body);
            res.status(200).json({
                success: true,
    ...
    async getUserCompletions(req, res) {
        try {
            const completions = await resourceService.getUserCompletions(req.user._id);
            res.status(200).json({
                success: true,
                data: completions
            });
